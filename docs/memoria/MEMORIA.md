...
- **26/09 (reparo dos 3 scripts ops estalados)**: `_mk_cria_anuncios.py` (`painel.STORE` inexistente → `bot.STORE`; `STORE._post` inexistente → `requests.post`; removidas chaves junk `accepts_offers/acepta_ofertas/is_featured` que dariam 400, `category`, fallback `painel._iteminfo`, e `tipo` capitalizado que quebrava o label do card), `_mk_teste.py` (mesmos fixes + tira `category`), `_mk_cria_teste.py` (tira `bot._mk_itemcategory` deletado + `category`). Validado: py_compile OK, checagem AST (7 refs `bot.*`/`painel.*` todas existem), suites 87+26 OK. Scripts seguem gitignored (sem commit de código).

- **26/09 (reparos pós-reorg)**: reescrito `tools/_mk_criar_troca.py` (era stub de 4 linhas com `import p` truncado; agora cria anúncios VIVOS espelhando o payload atual do publicar, via `bot.STORE`, sem Wiki; validado via py_compile + checagem estática de refs) e removido cabeçalho SEO duplicado no LEMBRETE (pré-existente). Não commitado o script (gitignored, como os demais `_mk_*`).
...
- **v2.10.21 (26/09, filtros do MARKTRADE CONSERTADOS — pendência desde v2.10.4-dados)**: o dono pediu "arrume os filtros". Causa raiz: o payload da `/api/troca` **não tinha o campo `voc`**, então clicar em qualquer chip de Vocação (Knight/Paladin/Sorcerer/Druid/Monk) caía no `continue outer` e **zerava a vitrine** ("Nenhum anúncio corresponde aos filtros"). Fix: novo helper `_mk_vocs(item_name)` (painel.py) que normaliza vocações para `["knight",...]` (fonte 1: ficha local `mk_itens` campo `vocacao` "Knights"/"Sorcerers"...; fonte 2 fallback: Wiki `vocrequired`; sem fonte → `[]`), campo `"voc"` adicionado ao payload da `/api/troca`, e `applyFilters` do `troca.html` agora casa por **inclusão na lista** em vez de igualdade exata. De quebra: **botão "Limpar filtros" do estado vazio não funcionava** (listener preso num `<template>` destacado; agora anexa no botão inserido) e `hideOffers` saiu do `restoreState` (não há mais switch na UI; estado legado travava ofertas ocultas sem controle). VERSION **painel.py 2.10.21** (bot.py segue 2.10.20, sem mudança funcional). Validado: py_compile OK; test_marketplace **89 OK** (4 novos: `_mk_vocs` ficha local/Wiki-fallback/sem-fonte + payload `/api/troca` traz `voc`) + test_coins OK; node --check do JS inline OK; simulação node dos filtros OK (sem filtro 2, voc=sorcerer → Sanguine Coil, voc=knight → War Hammer, venda+auroria → War Hammer). Pendência: re-deploy no Render (painel 2.10.21) + GitHub Pages (troca.html) e validar ao vivo os chips de Vocação.

- **26/09 (auditoria leve de segurança de sexta-feira, DOCUMENTAL)**: revisão AppSec estática completa do bapzx (bot.py, painel.py, rbac.py, storage.py, legais.py, mk_itens.py, track.js + páginas do portfolio) + pip-audit. Resultado: **Críticos 0, Altos 0, Médios 2, Baixos 5, Info 5, PASS 14/15, N/A 3**. Sem alteração de código nem bump de versão (só documentação em `BAPZX_SECURITY_AUDIT_2026-09-26.md`). Achados: M1 CSP `script-src 'unsafe-inline'` (JS inline do admin; sem XSS conhecido); M2 SSRF latente em `_mk_sprite_host` quando `MK_SPRITE_HOST` listar supabase/cloudinary (GET em URL arbitrária do usuário; env não setada → inerte hoje; fix: allowlist de host + MIME image/* + limite de tamanho); B1 requirements.txt sem pin; B2 webhook MP sem assinatura (mitigado por reconsulta + rate limit 60/min); B3 rate limit em memória do processo (zeda no restart/pod); B4 /api/track aceita ~11MB de body; B5 /health+x-render-origin-server divulgam versão; I1 /api/grupos reflete Origin sem _cors_ok (dado público); I2 /api/item?debug=1 expõe diag Wiki; I3 DASHBOARD_KEY comparada com == (timing); I4 fallback storage em pedidos.json (resiliência, sem resync); I5 confirmar `state` no OAuth. Próximo: nomeação dos fixes M1/M2/B1-B4 na ordem que o dono aprovar (ou manter como documentado).

- **v2.10.20 (25/09, 3 bugs do MARKTRADE encontrados pelo dono ao publicar)**:
  1. **"Aceito ofertas" pedia dinheiro**: o JS `appl()` do form de publicar NUNCA gravava o modo no input hidden `mk_modo_preco` (ficava sempre `preco_fixo`), então o submit com "Aceito ofertas" e sem preço caía no erro "Defina o preço em gp". Fix: `modo.value = v` no `appl()`. O servidor já tratava `modo_preco=aceito_ofertas` corretamente (aceita_ofertas = ... or modo_preco == "aceito_ofertas").
  2. **Preço aceitava letras**: campo `preco` era `type='text'` sem filtro. Fix: `inputmode='numeric'` + listener JS `somenteDigitos()` que apaga não-dígitos (limite 9 dígitos).
  3. **WhatsApp aparecendo no card da vitrine** (`troca.html`): o card renderizava a `.mk-contact-row` com o contato, mas o dono quer o número **só na tela de detalhe** (`anuncio.html`). Fix: removi `contactHtml`/`.mk-contact-row` da montagem do card e o CSS órfão `.mk-contact*` do troca.html (a página de detalhe continua mostrando via `.mini-contact` nos relacionados e `.d-contact-box` no bloco principal).
  VERSION bot+painel **2.10.20**. 2 testes novos em test_marketplace: `test_publicar_aceito_ofertas_sem_preco_nao_exige_dinheiro` (não bloqueia e grava `aceita_ofertas=True`) e `test_publicar_modo_preco_fixo_sem_preco_pede_dinheiro` (bloqueia 302 pro /cliente/troca). **85 OK** marketplace + **26 OK** coins. Pendência: re-deploy no Render (precisa do RR do dono) para subir v2.10.20 + o MK_SPRITE_HOST em lista (`cloudinary,supabase`). Portfolio: remover o contato do card foi commitado no bapzx-portfolio (troca.html).

- **v2.10.19 (25/09, cascata de hospedagem do sprite: Supabase Storage → Cloudinary → hotlink direto)**: o dono pediu "da pra fazer os 3?" — sim, como cascata. Novo `_mk_sprite_host(url)` em bot.py (chamado no `cliente_troca_publicar` depois do `_mk_itemsprite`): a env `MK_SPRITE_HOST` aceita **LISTA separada por vírgula na ordem de prioridade** (ex.: `cloudinary,supabase` — tenta o primeiro; falhou pula pro próximo; no fim cai no hotlink direto). Provedores: `supabase` (baixa o GIF do Wiki e sobe num bucket público já criado `mk-sprites`, devolve `.../storage/v1/object/public/mk-sprites/...`), `cloudinary` (sobe via `image/upload` — NÃO `image/fetch`, que está quebrado porque o Wiki devolve 403 — exigindo envs `CLOUDINARY_CLOUD_NAME`+`API_KEY`+`API_SECRET` com assinatura SHA-1). **Nunca derruba.** `_mk_sprite_cdn` de painel.py neutralizado (pass-through). VERSION bot+painel **2.10.19**. Validado ao vivo: bucket `mk-sprites` criado; Mace e War Hammer subidos e renderizando 32×32 no Playwright; Golden Helmet testado em lista `cloudinary,supabase` (sem envs do Cloudinary local caiu no Supabase) e `supabase,cloudinary`. Testes da cascata: `test_mk_sprite_host_sem_enum_volta_direto`, `_supabase_sobe_no_bucket`, `_supabase_falha_cai_no_direto`, `_cloudinary_sem_env_cai_no_direto`, `_lista_tenta_ordem_e_cai_no_direto`, `_lista_ignora_provedor_desconhecido` — **83 OK** (agora 85 com os 2 de v2.10.20). **Config no Render**: `MK_SPRITE_HOST=cloudinary,supabase` (lista) é o recomendado; `MK_IMG_CDN_BASE` agora ignorada.
