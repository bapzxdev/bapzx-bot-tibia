...
- **v2.10.4 (23/09, código: publicar padronizado + categoria + contato obrigatório + sprite fixo)**: pedido do dono sobre o MARKTRADE: (1) **nome do item em title case** ao publicar (WAR HAMMER → War Hammer; conectivos de/do/da/da/the/of/a/em etc. em minúsculo via `_mk_title_case`); (2) **select de Categoria** no publicar ("Automático (Wiki Tibia)" como padrão → se não escolher, `_mk_itemcategory` consulta o Tibia Wiki por `prop=categories` e mapeia termos → Soul Core/Make Believe/Rares/Primal Ordeal/Fansite/Soul War/Rotten Blood/House; nunca estoura, retorna "" em falha); (3) **Contato obrigatório** (campo `required` + validação no POST com flash+redirect; label "Contato * (discord/telegram/whypixels)"); (4) **sprite fixo**: `_mk_itemsprite` agora tenta de novo com `_mk_title_case(nome)` quando a 1ª busca falha (WAR HAMMER/anúncios antigos voltam a ter foto). POST do publicar reordenado (valida mundo antes de chamar rede da categoria). painel.py `/api/troca` agora devolve `categoria` e `contato`; troca.html passou a usar `a.categoria` no filtro icat (antes `a[k]` — filtro "Categoria do item" funcionava errado) e o card renderiza chip de categoria + linha de contato (com ✉). Testes: +5 novos (title case, categoria wiki, contato obrigatório, normalização+cat manual, sprite auto com categoria). VERSION bot+painel **2.10.4** (bump real de código após o rótulo v2.10.4 anterior que era só dados/UI). py_compile OK; test_marketplace **45 OK**; test_coins **26 OK**; JS check troca.html OK; navegador mock validado (2 anúncios, chip categoria, contato, sprite e filtro icat). **Próximo passo (dono): re-deploy no Render (v2.10.4 no /health) e testar ao vivo: publicar com "WAR HAMMER" (ver nome "War Hammer" + foto + categoria automática) e sem contato (deve rejeitar).**
